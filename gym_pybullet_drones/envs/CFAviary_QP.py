...
# Import QP-CBF controller for velocity-based control
...
class CFAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        """Initialization of an aviary environment for use of Crazyflie controller with optional CBF.
        ...
        # Initialize CBF controller if requested
        self.use_cbf = use_cbf
        if self.use_cbf:
            self._initialize_cbf_controller(cbf_params)

        self._initalize_cffirmware()
    
    def _initialize_cbf_controller(self, cbf_params):
        """Initialize the QP-CBF controller for velocity-based safety.
        
        Parameters
        ----------
        cbf_params : dict
            Parameters for CBF controller {'gamma': float, 'obs_radius': float}.
        """
        if cbf_params is None:
            cbf_params = {'gamma': 1.5, 'obs_radius': 0.1}
        
        # Load drone configuration
        drone_cfg_path = os.path.join(os.path.dirname(__file__), 'CBF_CONTROL', 'drone1.json')
        if not os.path.exists(drone_cfg_path):
            print(f"[WARNING] Drone config not found at {drone_cfg_path}, using default parameters")
            self.drone = None
        ...
        self.qp_controller = QP_Controller_Drone(
            gamma=cbf_params.get('gamma', 1.5),
            obs_radius=cbf_params.get('obs_radius', 0.1)
        )
        
        print(f"[CFAviary] CBF controller initialized with gamma={cbf_params.get('gamma', 1.5)}, obs_radius={cbf_params.get('obs_radius', 0.1)}")
    ...
    def step(self, i, obstacle_positions=None, obstacle_velocities=None):
        '''Step the firmware_wrapper class and its environment with optional CBF safety filter.
        ...
        Args: 
            ...
            obstacle_positions: list of obstacle positions [[x1,y1,z1], [x2,y2,z2], ...] (optional, for CBF)
            obstacle_velocities: list of obstacle velocities [[vx1,vy1,vz1], [vx2,vy2,vz2], ...] (optional, for CBF)
            
        Returns:
            obs, reward, terminated, truncated, info (standard gym interface)
        ...
        while self.tick / self.firmware_freq < t + self.ctrl_dt:
            ...
            # DEBUG: Print setpoint velocities BEFORE CBF
            if self.tick % 500 == 0:  # Print every 2 seconds (500 ticks @ 500Hz)
                print(f"\n[CFAVIARY - BEFORE CBF] tick={self.tick}, time={self.tick / self.firmware_freq:.2f}s")
                print(f"[CFAVIARY] Current position: [{cur_pos[0]:.3f}, {cur_pos[1]:.3f}, {cur_pos[2]:.3f}]")
                print(f"[CFAVIARY] Current velocity: [{cur_vel[0]:.3f}, {cur_vel[1]:.3f}, {cur_vel[2]:.3f}]")
                print(f"[CFAVIARY] Setpoint position command: [{self.setpoint.position.x:.3f}, {self.setpoint.position.y:.3f}, {self.setpoint.position.z:.3f}]")
                print(f"[CFAVIARY] Mellinger velocity output (BEFORE CBF): vx={self.setpoint.velocity.x:.3f}, vy={self.setpoint.velocity.y:.3f}, vz={self.setpoint.velocity.z:.3f}")

            # Apply CBF safety filter if enabled (modifies setpoint velocities)
            # Only apply after initial stabilization (first 0.5 seconds)
            if self.use_cbf and obstacle_positions is not None and self.tick > self.firmware_freq * 0.5:
                self._apply_cbf_filter(cur_pos, cur_vel, cur_rpy, obstacle_positions, obstacle_velocities)
                
                # DEBUG: Print setpoint velocities AFTER CBF
                if self.tick % 500 == 0:
                    print(f"[CFAVIARY - AFTER CBF] Filtered velocity output: vx={self.setpoint.velocity.x:.3f}, vy={self.setpoint.velocity.y:.3f}, vz={self.setpoint.velocity.z:.3f}")
    ...
    def _apply_cbf_filter(self, pos, vel, rpy, obstacle_positions, obstacle_velocities):
        """Apply QP-CBF safety filter to modify velocity setpoints.
        ...
        obstacle_velocities : list
            List of obstacle velocities [[vx1,vy1,vz1], [vx2,vy2,vz2], ...]
        """
        if self.drone is None or self.qp_controller is None:
            return
        
        # Update drone state for CBF controller (angular velocities computed internally)
        self.drone.update_state(pos.tolist(), vel.tolist(), rpy.tolist(), self.firmware_dt)
        
        # Extract reference velocities from setpoint
        u_ref = np.array([
            [self.setpoint.velocity.x],
            [self.setpoint.velocity.y],
            [self.setpoint.velocity.z],
            [self.setpoint.attitudeRate.yaw / self.RAD_TO_DEG]  # Convert back to rad/s
        ])
        
        # DEBUG: Show what CBF sees
        if self.tick % 500 == 0:
            print(f"\n[CBF INPUT]")
            print(f"[CBF] u_ref from Mellinger: vx={u_ref[0,0]:.3f}, vy={u_ref[1,0]:.3f}, vz={u_ref[2,0]:.3f}")
            print(f"[CBF] Drone position: [{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}]")
            print(f"[CBF] Obstacles: {obstacle_positions}")
        
        # Set reference control
        self.qp_controller.set_reference_control(u_ref)
        
        # Setup and solve QP with obstacles
        if obstacle_velocities is None:
            obstacle_velocities = [[0, 0, 0]] * len(obstacle_positions)
        
        self.qp_controller.setup_QP(self.drone, obstacle_positions, obstacle_velocities)
        self.qp_controller.solve_QP(self.drone)
        
        # Get safe velocity commands
        u_safe = self.qp_controller.get_optimal_control()
        
        # DEBUG: Show CBF output
        if self.tick % 500 == 0:
            print(f"[CBF OUTPUT] u_safe from QP: vx={u_safe[0]:.3f}, vy={u_safe[1]:.3f}, vz={u_safe[2]:.3f}")
        
        # Update setpoint with safe velocities
        self.setpoint.velocity.x = u_safe[0]
        self.setpoint.velocity.y = u_safe[1]
        self.setpoint.velocity.z = u_safe[2]
        self.setpoint.attitudeRate.yaw = u_safe[3] * self.RAD_TO_DEG  # Convert to deg/s
